...
import SearchLiveEureka from "./SearchLiveEureka";
...
const AddStrategyMain_Eureka = () => {
  ...
  const [formValues, setFormValues] = useState({
    strategyName: "",
    description: "",
    assetClasses: [{ name: "eureka", underlyings: [] }],
  });
      ...
      try {
        if (id) {
          const data = await ServerRequest({
            method: "get",
            URL: `/strategy/getoneEureka/?id=${id}`,
          });
  ...
  const handleSubmit = async () => {
    ...
    navigate("/accounts/dashboard/eureka/strategy", {
  ...
  const handleUpdate = async () => {
    ...
    const data1 = await ServerRequest({
      method: "post",
      URL: `/strategy/insertEureka?strategy_id=${id}`,
      data: { ...formValues, email_id: email_id },
    });
...
export { AddStrategyMain_Eureka, StrategyCreated_Eureka };
