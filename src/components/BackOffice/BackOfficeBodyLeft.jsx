import React from "react";
import BackOfficeDashboardEdit from "./BackOfficeDashboardEdit";
import BackOfficeDashboardPreview from "./BackOfficeDashboardPreview";
import BackOfficeAddFund from "./BackOfficeAddFund";

const BackOfficeMainHeader = ({ setCurrentPage, currentpage, fundDetail }) => {
  if (currentpage == "add" || currentpage == "created") return <></>;
  return (
    <div className="swift-back-office-main-header">
      <div className="swift-back-office-main-header-row-1">
        <div className="swift-back-office-main-header-row-1-left">
          <p>Copy Fund Profile Link</p>
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <rect width="20" height="20" fill="url(#pattern0_4143_556)" />
            <defs>
              <pattern
                id="pattern0_4143_556"
                patternContentUnits="objectBoundingBox"
                width="1"
                height="1"
              >
                <use href="#image0_4143_556" transform="scale(0.0078125)" />
              </pattern>
              <image
                id="image0_4143_556"
                width="128"
                height="128"
                href="data:image/png;base64,"
              />
            </defs>
          </svg>
        </div>
        <div
          className="swift-back-office-main-header-row-1-right"
          style={{ cursor: "pointer" }}
        >
          <p
            onClick={() => setCurrentPage("edit")}
            style={{ fontWeight: currentpage == "edit" ? "700" : "400" }}
          >
            Edit
          </p>
          <p
            onClick={() => setCurrentPage("preview")}
            style={{ fontWeight: currentpage == "preview" ? "700" : "400" }}
          >
            Preview
          </p>
        </div>
      </div>
      {currentpage == "preview" && fundDetail.fund_description && (
        <div className="swift-back-office-main-header-row-2">
          <p>Get in touch with the team</p>
          <p style={{ fontSize: "10px" }}>
            Fund last viewed on
            {new Date(fundDetail.fund_details_updated)
              .toLocaleDateString("en-GB", {
                day: "numeric",
                month: "long",
                year: "numeric",
              })
              .replace(
                /(\d+)(?= )/,
                (day) =>
                  day +
                  ["th", "st", "nd", "rd"][
                    day % 10 > 3 || Math.floor((day % 100) / 10) === 1
                      ? 0
                      : day % 10
                  ]
              )}
          </p>
          <p style={{ fontSize: "10px" }}>
            Fund last updated on{" "}
            {new Date(fundDetail.fund_details_updated)
              .toLocaleDateString("en-GB", {
                day: "numeric",
                month: "long",
                year: "numeric",
              })
              .replace(
                /(\d+)(?= )/,
                (day) =>
                  day +
                  ["th", "st", "nd", "rd"][
                    day % 10 > 3 || Math.floor((day % 100) / 10) === 1
                      ? 0
                      : day % 10
                  ]
              )}
          </p>

          <p>
            Firm Assets{" "}
            <span style={{ fontWeight: "700" }}>{fundDetail.firm_assets}</span>
          </p>
          <p>
            Strategy Assets{" "}
            <span style={{ fontWeight: "700" }}>
              {fundDetail.strategy_assets}
            </span>
          </p>
        </div>
      )}
    </div>
  );
};

const BackOfficeBodyLeft = ({
  currentpage,
  setCurrentPage,
  setAddFundName,
  addFundName,
  funds,
  selectedfund,
  setReset,
  setSelectedFund,
}) => {
  const fundDetail = funds.find((fund) => fund.id === selectedfund);

  return (
    <div className="swift-back-office-body-left">
      {(currentpage == "edit" || currentpage == "preview") && (
        <BackOfficeMainHeader
          currentpage={currentpage}
          setCurrentPage={setCurrentPage}
          fundDetail={fundDetail}
        />
      )}
      {currentpage == "edit" && (
        <BackOfficeDashboardEdit
          fundDetail={fundDetail}
          setReset={setReset}
          setCurrentPage={setCurrentPage}
        />
      )}
      {currentpage == "preview" && (
        <BackOfficeDashboardPreview fundDetail={fundDetail} />
      )}
      {(currentpage == "add" || currentpage == "created") && (
        <BackOfficeAddFund
          setAddFundName={setAddFundName}
          addFundName={addFundName}
          currentpage={currentpage}
          setCurrentPage={setCurrentPage}
          setReset={setReset}
          setSelectedFund={setSelectedFund}
        />
      )}
    </div>
  );
};

export default BackOfficeBodyLeft;
